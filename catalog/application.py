...
def showRecentBooks():
    ...
    return render_template('listBooks.html', book_type='Recently Added',
                           books=recentBooks)
...
@app.route('/books/all')
def showAllBooks():
    books = session.query(Book).order_by(Book.genre.asc(),
                                         Book.title.asc()).all()
...
def showMyBooks():
    ...
    books = session.query(Book).filter(Book.user_id ==
                                       login_session['user_id']).order_by(
                                       Book.genre.asc(), Book.title.asc())
...
def showGenreBooks(book_genre):
    ...
    return render_template('listBooks.html', book_type=editGenre,
                           books=genreBooks)
...
def addBook():
    ...
    if request.method == 'POST':
        ...
        newBook = Book(title=request.form['title'], urlSafeTitle=urlSafe,
...
def editBook(book_title, book_id):
    ...
    if editedBook.user_id != login_session['user_id']:
        return ("<script>function myFunction() {alert('You are not" +
                " authorized to edit this book. Please create your own" +
                " book in order to edit.');}</script><body" +
                " onload='myFunction()'>")
    if request.method == 'POST':
        ...
        session.commit()
        return redirect(url_for('showBookInfo', book_title=editedBook.title,
